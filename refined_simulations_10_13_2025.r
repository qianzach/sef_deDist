...
matrixName = paste0("./PowerMatrix-", DataType, "-",de_type ,"-varying-cells-n1-", n1, "-n2-", n2, "-p-", p, ".csv" )
# changed to ./density_estimation/simulations/logs2025/figures/post_changes_08_18_25 for most recent
...
df_pw
matrixName = paste0("./PowerMatrix-", DataType, "-",de_type ,"-varying-cells-n1-", n1, "-n2-", n2, "-p-", p, ".csv" )
# changed to ./density_estimation/simulations/logs2025/figures/post_changes_08_18_25 for updated
# names(df_pw)[names(df_pw) == 'ttest'] <- 'ftest' #used to look at ftest instead of ttest
write_csv(df_pw, matrixName)
# df_pw = read.csv(matrixName)
df_pw = df_pw[1:10,]
df_pw2 = reshape::melt( df_pw, id.vars = "effect_size")
...
for (idx_b in 1:length(b_thetas)) {
  ...
  outputPB = foreach(i = 1:maxIter, .packages = c("ggplot2","truncnorm")) %dopar% {
    ...
    wilcox_testPB = simulatePseudobulkRealistic(mu = mu, mu2 = mu2, sigma_sq = sigma_sq, theta = theta, b_theta = b_theta,
                                           b_pi = b_pi, pi = pi, pi2 = pi2,
                                           distribution = DataType,test_type = "wilcoxon", de_type = de_type, n1 = n1, n2 = n2, idx = i)
    list(t_testPB = t_testPB, ksPB = ksPB, f_testPB, wilcox_testPB)
  }
  ...
  pvMat_ttest    <- cbind(pvMat_ttest, sapply(outputPB, function(x) x$t_testPB$pval))
  pvMat_ks    <- cbind(pvMat_ks, sapply(outputPB, function(x) x$ksPB$pval))
  pvMat_ftest    <- cbind(pvMat_ttest, sapply(outputPB, function(x) x$f_testPB$pval))
  pvMat_wilcox    <- cbind(pvMat_ttest, sapply(outputPB, function(x) x$wilcox_testPB$pval))
}
stopCluster(cl) 
