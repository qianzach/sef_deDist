# EXECUTING THE NEW SIMULATION SET UP
rm(list = ls())  
library(dplyr) 
library(truncnorm)
library(ggplot2)
...
GraphName = paste0("./density_estimation/simulations/logs2025/figures/varying_cells/carrier-density-", DataType, "-n1-", n1, "-n2-", n2, "-p-", p, "-K-", K, ".pdf" )
...
GraphName = paste0("./density_estimation/simulations/logs2025/figures/varying_cells/carrier-density-", DataType, "-n1-", n1, "-n2-", n2, "-p-", p,".pdf" )
pdf(GraphName, width = 5, height = 5)
print(test$carrier_plt)
dev.off() 
...
GraphName = paste0("./density_estimation/simulations/logs2025/figures/varying_cells/varying_cell_cts_qqplot-", DataType, "-n1-", n1, "-n2-", n2, "-p-", p, "-K-", K, ".pdf" )
pdf(GraphName, width = 5, height = 5)
...
GraphName  = paste0("./density_estimation/simulations/logs2025/figures/varying_cells/", DataType, "-carrier-density-n1-", n1, "-n2-", n2, "-p-", p, "-K-", K, ".pdf" )
pdf(GraphName, width = 5, height = 5)
print(test$carrier_plt)
dev.off() 

GraphName  = paste0("./density_estimation/simulations/logs2025/figures/varying_cells/", DataType, "-sef-models-n1-", n1, "-n2-", n2, "-p-", p, "-K-", K, ".pdf" )
pdf(GraphName, width = 5, height = 5)
...
pvVec_margin <- sapply(output, function(x) x$pval_1)
gg_qqplot(unlist(pvVec_margin))
GraphName = paste0("./density_estimation/simulations/logs2025/figures/varying_cells/varying_cell_cts_qqplot-", DataType, "-n1-", n1, "-n2-", n2, "-p-", p,".pdf" )
pdf(GraphName, width = 5, height = 5)
...
output <- pbmclapply(1:maxIter, function(i) {
  tryCatch({
    simulateNBRealistic(
      ...
      sigma_sq1 = sigma_sq1, sigma_sq2 = sigma_sq2,
      p = 2, idx = i, de_type = de_type,
      n1 = n1, n2 = n2
    )
  }, error = function(e) {
...
pvVec_margin <- sapply(output, function(x) x$pval_1)
gg_qqplot(unlist(pvVec_margin)) + ggtitle("Negative Binomial Model")

GraphName = paste0("./density_estimation/simulations/logs2025/figures/varying_cells/varying_cell_cts_qqplot-", DataType, "-n1-", n1, "-n2-", n2, "-p-", p, ".pdf" )
pdf(GraphName, width = 5, height = 5)
realistic_qqplot = gg_qqplot(unlist(pvVec_margin)) + ggtitle("NB Model")
print(realistic_qqplot)
dev.off() 

# print out the carrier density example
test = simulateNBRealistic(
  mu1 = mu1, mu2 = mu2,
  theta1 = 2.5, theta2 = 2.5,
  sigma_sq1 = sigma_sq1, sigma_sq2 = sigma_sq2,
  p = 2, idx = i, de_type = de_type,
  n1 = n1, n2 = n2
)
...
# change to post_changes_08_18_25
...
print(obj_pw)
GraphName = paste0("./density_estimation/simulations/logs2025/figures/varying_cells/Powerplot-", DataType,"-",de_type ,"-varying-cells-n1-", n1, "-n2-", n2, "-p-", p, ".pdf" )
pdf(GraphName, width = 5, height = 5)
print(obj_pw)
dev.off() 
test = simulateNormalRealistic(alpha1 = alpha1, alpha2 = alpha2, tau1 = tau1, tau2 = tau2, sigma1 = sigma1, sigma2 = sigma2, n1 = n1, n2 = n2,repID = repID,de_type = de_type, p = p, K = K,idx = 1)
...
matrixName = paste0("./density_estimation/simulations/logs2025/figures/varying_cells/PowerMatrix-", DataType, "-",de_type ,"-varying-cells-n1-", n1, "-n2-", n2, "-p-", p, ".csv" )
# changes to post_changes_08_18_25
# write_csv(df_pw, matrixName)
# df_pw = read.csv(matrixName)
df_pw = df_pw[1:10,]
df_pw2 = reshape::melt( df_pw, id.vars = "effect_size")
...
df_pw
matrixName = paste0("./density_estimation/simulations/logs2025/figures/varying_cells/PowerMatrix-", DataType, "-",de_type ,"-varying-cells-n1-", n1, "-n2-", n2, "-p-", p, ".csv" )
#change varying cells to post_change_08_18_2025 dir
# names(df_pw)[names(df_pw) == 'ttest'] <- 'ftest'
write_csv(df_pw, matrixName)
# df_pw = read.csv(matrixName)
df_pw = df_pw[1:10,]
df_pw2 = reshape::melt( df_pw, id.vars = "effect_size")
...
print(obj_pw)
dev.off() 
#  ---- ZERO-INFLATED NEGATIVE BINOMIAL MEAN (and variance technically)  ----
args <- commandArgs(trailingOnly = TRUE)
mu = 10
mu2s = seq(10, 13, 0.25)
theta = 5
...
df_pw = data.frame(effect_size = (mu2s - mu), margin = margin_sef,
                   ...
                   ttest = colMeans(pvMat_ttest < q),
                   ks = colMeans(pvMat_ks < q))
matrixName = paste0("./density_estimation/simulations/logs2025/figures/post_change_08_18_2025/PowerMatrix-", DataType, "-",de_type ,"-varying-cells-n1-", n1, "-n2-", n2, "-p-", p, ".csv" )
# write_csv(df_pw, matrixName)
df_pw = df_pw[1:10,]
df_pw2 = reshape::melt( df_pw, id.vars = "effect_size")
colnames(df_pw2) = c("effect_size", "Methods", "Power")
...
obj_pw = ggplot(df_pw2, aes(x = effect_size, y = Power, group = Methods, col=Methods, shape=Methods, linetype = Methods)) +
  ...
  scale_color_manual(values = custom_colors)
print(obj_pw)
GraphName = paste0("./density_estimation/simulations/logs2025/figures/post_change_08_18_2025/Powerplot-", DataType,"-",de_type ,"-varying-cells-n1-", n1, "-n2-", n2, "-p-", p, ".pdf" )
pdf(GraphName, width = 5, height = 5)
print(obj_pw)
dev.off() 
...
df_pw = data.frame(effect_size = (b_thetas*theta/theta), margin = margin_sef,
                   ...
                   ttest = colMeans(pvMat_ttest < q),
                   ks = colMeans(pvMat_ks < q))
matrixName = paste0("./density_estimation/simulations/logs2025/figures/post_change_08_18_2025/ftest-PowerMatrix-", DataType, "-",de_type ,"-varying-cells-n1-", n1, "-n2-", n2, "-p-", p, ".csv" )
# write_csv(df_pw, matrixName)
# change varying_cells to post_change_08_18_2025
df_pw = read.csv(matrixName)
df_pw = df_pw[1:10,]
df_pw2 = reshape::melt( df_pw, id.vars = "effect_size")
colnames(df_pw2) = c("effect_size", "Methods", "Power")
...
GraphName = paste0("./density_estimation/simulations/logs2025/figures/post_change_08_18_2025/ftest-Powerplot-", DataType,"-",de_type ,"-varying-cells-n1-", n1, "-n2-", n2, "-p-", p, ".pdf" )
pdf(GraphName, width = 5, height = 5)
print(obj_pw)
dev.off() 

#  ---- (DO NOT RUN) ZERO-INFLATED NEGATIVE BINOMIAL MIXTURE  ----
args <- commandArgs(trailingOnly = TRUE)
mu = 10; mu2 = 12
theta = 5
n1 = 100; n2 = 100
...
bad_test_cases = array(NA, dim = length(b_pis))

test = simulateZINB(mu = mu, mu2 = mu2, sigma_sq = sigma_sq, theta = theta, b_theta = b_theta,
                          b_pi = 0.85, pi = pi, pi2 = pi2,
                          repID, n1 = n1, n2 = n2, de_type = de_type, K = NULL, p = p,idx = 46)

for (idx_b in 1:length(b_pis)) {
  b_pi <- b_pis[idx_b]
  print(paste0("mixture parameter b_pi = ", b_pi))
  ...
  output <- foreach(i = 1:maxIter, .packages = c("ggplot2","truncnorm")) %dopar% {
    tryCatch({
      simulateZINB(mu = mu, mu2 = mu2, sigma_sq = sigma_sq, theta = theta, b_theta = b_theta,
                   b_pi = b_pi, pi = pi, pi2 = pi2,
                   repID, n1 = n1, n2 = n2, de_type = de_type, K = NULL, p = p,idx = i)
    }, error = function(e) {
      message(sprintf("Error in iteration %d: %s", i, e$message))
      return(NULL)
    })
  }
  output1 <- Filter(function(x) {
    !is.null(x) &&
      !is.null(x$sef_gp1) && !is.null(x$sef_gp2) &&
      isTRUE(x$sef_gp1$converged) &&
      isTRUE(x$sef_gp2$converged)
  }, output)
...
Allmethods = c("margin", "mom", "ttest", "ks")
init_b_pi = b_pis[1] #used purely for dataframe
q = 0.05 
margin_sef <- sapply(pvMat_margin, function(vec) mean(vec < q, na.rm = TRUE))
df_pw = data.frame(effect_size = b_pis, margin = margin_sef,
                   mom = colMeans(pvMat_mom < q, na.rm = T),
                   ttest = colMeans(pvMat_ttest < q),
                   ks = colMeans(pvMat_ks < q))
matrixName = paste0("./density_estimation/simulations/logs2025/figures/varying_cells/PowerMatrix-", DataType, "-",de_type ,"-varying-cells-n1-", n1, "-n2-", n2, "-p-", p, ".csv" )
...
obj_pw = ggplot(df_pw2, aes(x = effect_size, y = Power, group = Methods, col=Methods, shape=Methods, linetype = Methods)) +
  ...
  labs(x = expression("Effect Size of Mixture Proportion (" ~ b[p] ~ ")"), 
       y = "Power", 
       title = "Empirical Power (ZINB)") + 
  theme(legend.position = "bottom", panel.grid.minor = element_blank()) + 
  scale_color_manual(values = custom_colors)
