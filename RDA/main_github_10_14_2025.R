...
get_sc_bin_counts = function(Y, bin_edges, K){
  # single cell data (varying cell counts)
  n = length(Y)
...
filtration_method = function(sObj, gene_vector, exprMat, n_cores = parallel::detectCores() - 1) {
  ...
  stopifnot(ncol(exprMat) == ncol(sObj)) # check alignment
  
  donor_ids = as.character(sObj$donor_id)
  
  # run in parallel over genes
...
compute_mean_median_expr = function(exprMat, sort.by = c("median", "mean")) {
  # returns dataframe w/ K rows for K total genes
  sort.by = match.arg(sort.by) #choose one to sort by
  gene_indices = seq_len(nrow(exprMat)) #total number of genes in order
  gene_names = rownames(exprMat)
  
  # compute mean, median in parallel
  ...
  stats_df = stats_df[order(-stats_df[[sort.by]]), ] #sort by one of the two options here
  return(stats_df)
}
...
sef_regression = function(exprMat, sObj_meta, donor_list, p = 2, plot_flag = F, h = NULL){
  ...
  # sum first to avoid numerical issue
  S1sum = colSums(Smat1)
  S2sum = colSums(Smat2)
